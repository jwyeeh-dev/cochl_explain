# Copy from Intel Corporation, Changed by Jaeyeong Hwang
...
class ReciproCam:
    def __init__(self, feature_net, head_net, device):
        ...
        self.device = device
        self.feature = None
        self.softmax = keras.activations.softmax(dim=1)
        self.gaussian = tf.Tensor(
            [
                [1 / 16.0, 1 / 8.0, 1 / 16.0],
                [1 / 8.0, 1 / 4.0, 1 / 8.0],
                [1 / 16.0, 1 / 8.0, 1 / 16.0],
    ...
    def _mosaic_feature(self, feature_map, is_gaussian=False):
        ...
        new_features = tf.zeros(height * width, num_channel, height, width).to(
            self.device
        )
    ...
    def _weight_accum(self, mosaic_predict, class_index, height, width):
        cam = tf.zeros(height, width).to(self.device)
        for i in range(height):
            for j in range(width):
